...
const handleSubmit = () => {
  ...
  if (
    !roomInfo.value.building ||
    !roomInfo.value.organizationalUnit ||
    !roomInfo.value.roomNumber ||
    !roomInfo.value.organizationalUnit ||
    !roomInfo.value.street ||
    !roomInfo.value.zone
  ) {
  ...
  <div class="container">
    ...
    <v-card class="list">
      ...
      <form @submit.prevent="submit" v-if="roomInfo">
        <v-text-field v-model="roomInfo.roomNumber" label="Nazwa sali"></v-text-field>
        ...
        <v-btn class="me-4" type="submit" @click="handleSubmit"> Zatwierdź </v-btn>
      </form>
